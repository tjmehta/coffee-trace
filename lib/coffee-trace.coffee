...
module.exports = coffee_trace = (options={ascii_art:true})->
  (err) ->
    ...
    line_num = null
    line_col = null
    stack_split = if err.stack? then err.stack.split("\n").slice(1) else [];
    found = stack_split.some (line) ->
      # Find first .coffee file line in the stack
      line_split = line.match(/^ +at[^\/]+((?:\w+:\/\/)?[^:]+):(\d+):(\d+)/) or []
      filename = line_split[1]
      line_num = Number(line_split[2])
      line_col = Number(line_split[3])
